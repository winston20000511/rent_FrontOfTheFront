<template>
  <div class="house-details" v-if="!isLoading">
    <!-- 加載中錯誤處理 -->
    <div v-if="hasError" class="error-message">無法加載房屋資料，請稍後再試。</div>
    ...
    <div v-if="houseDetails" class="house-basic-info">
      ...
      <div v-if="houseDetails.atticAddition" class="info-item">
        <strong>是否為頂樓加蓋:</strong><span>{{ houseDetails.atticAddition ? '是' : '否' }}</span>
      </div>
    </div>
    ...
    <div class="furniture-services">
      ...
      <div v-for="(item, index) in furnitureServices" :key="index" v-show="item.available" class="icon-label">
        <img :src="baseUrl + 'src/assets/icon/house-on/' + item.src" 
             :alt="item.alt" :title="item.name" class="icon-image" />
        <span>{{ item.name }}</span>
      </div>
    </div>
    ...
    <div class="house-restrictions">
      ...
      <div v-for="(item, index) in houseRestrictions" :key="index" v-show="item.allowed" class="icon-label">
        <img :src="baseUrl + 'src/assets/icon/house-on/' + item.src" 
             :alt="item.alt" :title="item.name" class="icon-image" />
        <span>{{ item.name }}</span>
      </div>
      ...
      <div v-if="genderRestriction !== 0">
        <img :src="getGenderImage(genderRestriction)" 
             :alt="genderRestriction === 1 ? '男' : '女'" class="icon-image" />
      </div>
    </div>
  ...
  data() {
    return {
      ...
      furnitureServices: [
        ...
        { src: 'channel4.ico', alt: '第四台', name: '第四台', available: false },
  ...
  methods:{
    ...
    getGenderImage(genderRestriction) {
      if (genderRestriction === 1) {
          ...
          return new URL('@/assets/icon/house-on/female.ico' , import.meta.url).href;
      }
      return ''; // 如果是0，不顯示任何圖示
    },
