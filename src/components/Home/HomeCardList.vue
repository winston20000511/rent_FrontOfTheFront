<script setup>

</script>

<template>
<div class="container py-4 px-4 bg-body custom-shadow">
  <div class="row gy-4">
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
    <div class="col-12 col-md-6">
<<<<<<<< HEAD:src/components/search/HomeCardList.vue
        <div class="card card-shadow" style="width: 17rem;">
            <img src="/src/assets/img/view1.jpg" class="card-img-top" alt="...">
========
        <div class="card" style="width: 18rem;">
            <img src="../../assets/view1.jpg" class="card-img-top" alt="...">
>>>>>>>> dev:src/components/Home/HomeCardList.vue
            <div class="card-body">
                <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
            </div>
        </div>
    </div>
  </div>
</div>
</template>

<style scoped>
.custom-shadow {
    position: relative;
    margin: 0px 0px 0px 5px;
    box-shadow: -5px 0px 5px -3px rgba(0, 0, 0, 0.4) ; /* 浅灰色阴影 */
}
.card-shadow {
    position: relative;
    box-shadow: -5px 5px 5px -3px rgba(0, 0, 0, 0.4) ; /* 浅灰色阴影 */
}
</style>
