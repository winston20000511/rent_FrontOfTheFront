<template>
  <div class="booking-list-container">
      ...
      >
        ...
        <template v-if="col.field === 'houseAddress'" #body="slotProps">
          <div style="display: flex; align-items: center; gap: 0.5rem">
            ...
            <Button
              icon="pi pi-search"
              class="p-button-rounded p-button-text p-button-info action-button"
        ...
        <template v-if="col.field === 'userName'" #body="slotProps">
          <div class="relative inline-block">
            <!-- 主要顯示的姓名 -->
            <div
              v-if="slotProps.data.userStatus === 1"
            ...
            <!-- Popover 區塊 -->
            <div
              v-if="isPopoverVisible(slotProps.data.bookingId)"
            ...
            >
              <div class="flex justify-between items-center">
                ...
                <Button
                  icon="pi pi-copy"
                  @click="copyToClipboard(slotProps.data.userEmail)"
              ...
              <div class="flex justify-between items-center mt-2">
                ...
                <Button
                  icon="pi pi-copy"
                  @click="copyToClipboard(slotProps.data.userPhone)"
                  class="p-button-rounded p-button-text"
                />
              </div>
            </div>
        ...
        <template v-if="col.field === 'photos'" #body="slotProps">
          ...
          >
            <img
              :src="getPhoto(slotProps.data.photos)"
              alt="房屋圖片"
    ...
    >
      <div class="flex items-center gap-4">
        ...
        <span v-if="selectedBooking"
          >您確定要取消
          <b class="text-danger">編號: {{ selectedBooking.bookingId }}</b>
          的預約嗎?</span
        >
      </div>
      ...
      <template #footer>
        ...
        <Button
          ...
          icon="pi pi-times"
          severity="contrast"
          @click="cancelBookingDialog = false"
          :loading="loading"
        />
      </template>
    </Dialog>
    ...
    <!-- 處理預約彈窗 -->
    <Dialog
      v-model:visible="processBookingDialog"
      :style="{ width: '450px' }"
      header="處理預約確認"
      :modal="true"
      :contentStyle="{ fontSize: '18px' }"
    >
      <div class="flex items-center gap-4">
        ...
        <span v-if="selectedBooking">
          您是否同意
          <b class="text-primary">編號: {{ selectedBooking.bookingId }}</b>
          的預約？
        </span>
      </div>
      ...
      <div class="flex justify-between gap-4 mt-4">
        ...
        <Button
          ...
          severity="danger"
          @click="processBooking(selectedBooking, false)"
          style="width: 48%"
          :loading="loading"
        />
        ...
        <Button
          ...
          icon="pi pi-check"
          severity="success"
          @click="processBooking(selectedBooking, true)"
          style="width: 48%"
          :loading="loading"
          v-if="selectedBooking.userStatus === 1"
        />
      </div>
      <template #footer>
        <Button
          ...
          icon="pi pi-times"
          severity="contrast"
          @click="processBookingDialog = false"
        />
      </template>
    </Dialog>
  ...
  try {
    ...
    for (const booking of bookingList.value) {
      ...
      const photosResponse = await fetch(
        `${BASE_URL}/houses/getPhotos/${houseId}`,
        {
          headers: {
            authorization: `${TOKEN}`,
          },
      ...
      if (photosResponse.ok) {
        ...
        throw new Error(
          `狀態碼: ${photosResponse.status} ; 無法獲得 houseId=${houseId} 的圖片 ;`
        );
        booking.photos = [];
      }
    }
...
const useFilter = () => {
  ...
  if (activeTab.value === 0) {
    ...
    // 顯示 已取消
    filteredBookingList.value = list.filter(
      (item) =>
        item.status === 2 ||
        item.status === 3 ||
        item.status === 4 ||
...
const pendingCount = computed(() => {
  return Array.isArray(bookingList.value)
    ? bookingList.value.filter((booking) => booking.status === 0).length
    : 0;
});
...
const bookingedCount = computed(() => {
  return Array.isArray(bookingList.value)
    ? bookingList.value.filter((booking) => booking.status === 1).length
    : 0;
});
...
const togglePopover = (bookingId) => {
  activePopoverId.value =
    activePopoverId.value === bookingId ? null : bookingId;
};
